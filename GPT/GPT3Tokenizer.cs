...
namespace LisaCore.GPT
{
    public static class GPT3Tokenizer
    {
        ...
        public static string TrimTextUpToTokenLimit(string text, int maxTokenLimit)
        {
            ...
            int tokenCount = 0;
            int lastMatchEnd = 0; // This will keep track of the end position of the last match within our token limit
            ...
            foreach (Match match in matches)
            {
                ...
                if (tokenCount + newTokens.Count > maxTokenLimit)
                {
                    // If adding this match's tokens would exceed the limit, stop and return the substring up to the last match's end
                    break;
                }

                tokenCount += newTokens.Count;
                lastMatchEnd = match.Index + match.Length; // Update the position to the end of the current match
            }

            // Return the substring of the text up to the last match that fits within the token limit
            return text.Substring(0, lastMatchEnd);
        }
